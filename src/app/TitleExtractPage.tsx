...
import { TitleExtractTaskModal } from "./TitleExtractTaskModal";
...
const sampleImg =
  "https://images.unsplash.com/photo-1583743814966-8936f5b7be1a?w=200&h=240&fit=crop&auto=format";

function TitleExtractExamplePreview() {
  return (
    <div className="flex flex-col h-full p-3 gap-2">
      <div className="flex-1 rounded-md border border-border bg-white overflow-hidden min-h-[100px]">
        <img src={sampleImg} alt="" className="w-full h-full object-cover" />
      </div>
      <div className="rounded-md bg-amber-50 border border-amber-200/80 px-2 py-1.5 text-[10px] leading-snug text-muted-foreground line-clamp-3">
        Women&apos;s T-shirts Duck With Wine And Chocolates 100% Cotton Short Sleeve
      </div>
    </div>
  );
}

export function TitleExtractPage({ onBack }: { onBack: () => void }) {
  const [taskModalOpen, setTaskModalOpen] = useState(false);
  const { tasks, submitTask, listProps, DetailView } = useFeatureTaskDetail("title-extract");
    ...
    <>
      <div className="flex flex-1 flex-col min-h-0 overflow-hidden">
        <div className="flex items-center gap-3 px-6 h-14 border-b border-border shrink-0">
          ...
          <h1 className="text-[16px] font-semibold text-foreground">标题提取</h1>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-auto px-6 py-10 scrollbar-none">
          {tasks.length > 0 ? (
            ...
            <div className="max-w-[760px] mx-auto">
              <h2 className="text-[22px] font-semibold text-foreground text-center">玩转标题提取</h2>
              <p className="mt-2 text-center text-[13px] text-muted-foreground">
                上传商品图，AI 自动生成多语言刊登标题
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
                ...
                <div className="rounded-xl border border-border bg-card overflow-hidden min-h-[280px] flex flex-col">
                  ...
                  <p className="px-4 pt-2 pb-1 text-[13px] font-medium text-foreground">智能生成商品标题</p>
                  <div className="flex-1 min-h-[180px] bg-muted/30">
                    <TitleExtractExamplePreview />
                  </div>
                </div>
              </div>
            </div>
