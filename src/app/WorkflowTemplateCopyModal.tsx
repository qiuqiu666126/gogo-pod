...
export function WorkflowTemplateCopyModal({
  open,
  template,
  defaultCategory = "服饰",
  onClose,
  ...
  onClose: () => void;
  onCopied?: (category: WorkflowCategory) => void;
}) {
  const [name, setName] = useState("");
  const [category, setCategory] = useState<WorkflowCategory>(defaultCategory);
  ...
  const handleConfirm = () => {
    ...
    if (!trimmed) return;
    if (!category) return;
    addUserWorkflowTemplate(category, { name: trimmed, steps: template.steps });
    onCopied?.(category);
    onClose();
  };
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(520px,94vw)] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-2xl border border-border bg-card text-foreground shadow-2xl">
          ...
          <div className="space-y-4 px-6 py-6">
            ...
            <div className="grid grid-cols-[76px_1fr] items-center gap-3">
              ...
              <select
                ...
                className={`${fieldSelectClass} ${!category ? "text-muted-foreground" : ""}`}
              >
          ...
          <div className="flex items-center justify-end gap-3 border-t border-border/80 px-6 py-4">
            ...
            <button
              ...
              className="h-10 rounded-md bg-primary px-7 text-[14px] font-medium text-white hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              确定
            </button>
          </div>
