...
export function CrackImagePage({ onBack }: { onBack: () => void }) {
  const [taskModalOpen, setTaskModalOpen] = useState(false);
  const { submitTask, listProps, DetailView } = useFeatureTaskDetail("crack");
    ...
    <>
      <div className="flex flex-1 flex-col min-h-0 overflow-hidden">
        <div className="flex items-center gap-3 px-6 h-14 border-b border-border shrink-0">
          ...
          <h1 className="text-[16px] font-semibold text-foreground">图裂变</h1>
        ...
        <div className="flex-1 overflow-auto px-6 py-4 scrollbar-none">
          <FeatureTaskTable {...listProps} showParams />
        </div>
      </div>
